...
def check_dir(p, del_exist=False, print_info=False):
    if p.is_dir():
        if print_info:
            logger.info(f"already exists {p}")
        if del_exist:
            shutil.rmtree(p)
            logger.info(f"delete dir {p}")
...
def get_args():
    ...
    parser.add_argument("--train_set_name", type=str, default="train", help="")
    parser.add_argument("--val_set_name", type=str, default="val", help="")
    parser.add_argument("--ignore_classes", nargs="+", default=[], help="")
    ...
    parser.add_argument("--data_type", type=str, choices=["coco", "dota"], default="dota", help="")
...
def read_coco_data(ori_dir, del_files):
    pass
