"""
Filter results by different score_thr for image-format submissions.
e.g.:
Task
   |——————P1.txt
   └——————P2.txt
...
Each txt file is marked as follows:
    classname score x1 y1 x2 y1 x2 y2 x1 y2
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description='Filter results by for image-format submissions.')
...
def cal_scores(src_dir):
    ...
    for result in results:
        with open(result,'r') as f:
            ...
            for line in lines:
                splitline = line.strip().split(' ')
                category = CLASSES.index(splitline[0])
                # score = splitline[1]
